...
export const Marker: React.FC<MarkerProps> = ({ annotation }) => {
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (target && target !== document.body && target !== document.documentElement) {
      ...
      if (annotation._id) {
        saveCommentUpdate({ ...annotation, selector, offsetX, offsetY });
      }
    }
  };
  ...
  const saveCommentUpdate = async (updatedData: Annotation) => {
  ...
  const updateComment = async () => {
    ...
    if (updatedData) {
      await saveCommentUpdate(updatedData);
  ...
  const handleStatusChange = async (status: CommentStatus) => {
    ...
    setActiveAnnotationId(annotationKey);
    if (annotation._id) {
      await saveCommentUpdate({ ...annotation, status });
  ...
  const handleScreenChange = async (screenSize: ScreenSize) => {
    ...
    setActiveAnnotationId(annotationKey);
    if (annotation._id) {
      await saveCommentUpdate({ ...annotation, screenSize });
      toast.success('Screen visibility updated');
    }
  };
